...
class SearchController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        // Ambil parameter ratings dari request
        $selectedRatings = $request->input('ratings', []);

        // Ambil course_id berdasarkan rating yang dipilih
        if (!empty($selectedRatings)) {
            $filteredCourseIds = Reviews::whereIn('rating', $selectedRatings)
                ->pluck('class_id')
                ->toArray();
        } else {
            $filteredCourseIds = [];
        ...
        // Lakukan filter untuk courses berdasarkan rating yang dipilih
        $results = KelasTatapMuka::query()
            ->where('status', 1)
            ->whereIn('id', $kurikulumCourseIds)
            ->when(!empty($filteredCourseIds), function ($query) use ($filteredCourseIds) {
                return $query->whereIn('id', $filteredCourseIds);
            })
            ->paginate(6);
