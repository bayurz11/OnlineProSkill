    ...
    <script>
        function changeContent(element, event) {
            if (element.classList.contains('disabled')) {
                ...
                alert('Bagian ini terkunci.');
                return;
            }

            var fileUrl = element.getAttribute('data-link');
            var sectionId = element.getAttribute('data-id');
            var fileSrc = determineFileSrc(fileUrl, element.getAttribute('data-type'));
        ...
        function determineFileSrc(url, type) {
            if (url.includes('youtube')) return 'https://www.youtube.com/embed/' + extractId(url, 'youtube');
            if (url.includes('drive.google')) return determineDriveEmbed(url, type);
            if (type === 'pdf' || url.includes('uploads/')) return '/public/' + url;
            return '';
        }

        function extractId(url, type) {
            var regex = type === 'youtube' ? /(?:v=|youtu.be\/)([^"&?\/\s]{11})/ : /d\/([^"&?\/\s]+)/;
            return (url.match(regex) || [])[1];
        }

        function determineDriveEmbed(url, type) {
            const driveId = extractId(url, 'drive');
            switch (type) {
                case 'video':
